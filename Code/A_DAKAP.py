...
parser.add_argument('--twitter_epoch', dest='twitter_epoch', type=int, help='twitter_epoch', default=5)
parser.add_argument('--epoch', dest='epoch', type=int, help='epoch', default=6)
parser.add_argument('--template_id', dest='template_id', type=int, help='template_id', default=8)
...
epoches = args.epoch
twitter_epoch = args.twitter_epoch
...
class BertClassifier(nn.Module):
  ...
  def forward(self, X, dataadaption):
    input_ids, attention_mask, token_type_ids = X[0], X[1], X[2]

    ### conver token ids to the word embeddings: 
    embeddings = self.bert.bert.embeddings(input_ids) # bathch, max_len, hiddendim
    da_emb_one = dataadaption()
    da_emb = da_emb_one.expand(embeddings.shape[0], da_emb_one.shape[0], da_emb_one.shape[1])
    # add domain adaptive soft prompt
    embeddings = torch.cat([da_emb, embeddings], dim=1)

    ones_tensor = torch.ones((attention_mask.shape[0], 1)).to(device)
    zeros_tensor = torch.zeros((token_type_ids.shape[0], 1)).to(device)
    # after add domain adaptive soft prompt
    input_ids = torch.cat([ones_tensor, input_ids], dim=1).int()
    attention_mask = torch.cat([ones_tensor, attention_mask], dim=1).int()
    token_type_ids = torch.cat([zeros_tensor, token_type_ids], dim=1).int()
...
class DataAdaption(nn.Module):
    def __init__(self, hidden_size, device):
      super(DataAdaption, self).__init__()
      self.device = device
      self.davec = nn.Parameter(torch.randn(1, hidden_size).to(self.device), requires_grad=True).to(self.device)
      # self.davec = davec.expand(batch_size, davec.shape[0], davec.shape[1])
    def forward(self):
      return self.davec
...
if __name__ == '__main__':
  ...
  X_train_twitter, Y_train_twitter = A_load_data.load_twitter_train()
  ...
  Y_train_twitter = encoder.fit_transform(np.expand_dims(np.array(Y_train_twitter), axis=-1))
  Y_train_twitter = torch.from_numpy(Y_train_twitter)
  train_twitter = Data.DataLoader(dataset=BertDataset(X_train_twitter, Y_train_twitter), batch_size=batch_size, shuffle=True, num_workers=1)
  ...
  dataadaption_twitter = DataAdaption(hidden_size=hidden_size, device=device)
  dataadaption_weibo = DataAdaption(hidden_size=hidden_size, device=device)

  optimizer_twitter = optim.Adam(list(bc.parameters()) + list(dataadaption_twitter.parameters()), lr=args.learning_rate)
  optimizer = optim.Adam(list(bc.parameters()) + list(dataadaption_weibo.parameters()), lr=args.learning_rate)
  loss_fn = nn.CrossEntropyLoss()   

  ###  train on twitter
  bc.train()
  for epoch in range(twitter_epoch):
      loss_list = []
      for batch in train_twitter:
          optimizer_twitter.zero_grad()
          batch = tuple(p.to(device) for p in batch)

          pred = bc([batch[0], batch[1], batch[2]], dataadaption_twitter)
          loss = F.cross_entropy(pred, batch[3].argmax(dim=-1).to(device), reduction="mean")
          loss_list.append(loss.item())
          loss.backward()
          optimizer_twitter.step()
      print('[{}|{}]  loss:{:.4f}'.format(epoch+1, twitter_epoch, np.mean(np.array(loss_list))))
  ...
  for epoch in range(epoches):
      ...
      for batch in train:
          ...
          pred = bc([batch[0], batch[1], batch[2]], dataadaption_weibo)
          loss = F.cross_entropy(pred, batch[3].argmax(dim=-1).to(device), reduction="mean")
          loss_list.append(loss.item())
          loss.backward()
          optimizer.step()
  ...
  for batch in test:
      with torch.no_grad():
          ...
          preds = bc([batch[0], batch[1], batch[2]], dataadaption_weibo)
          preds = torch.softmax(preds, dim=-1)
